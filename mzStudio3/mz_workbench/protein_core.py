...
pass#print "Powered by protein core! Version " + __version__ + ' Last revised: ' + __lastRevision__
...
def create_gene_dicts(organism):
    pass#print "*"
    pass#print "Creating gene dictionaries..."
    gi2geneID = {}
    gi2geneName = {}
    geneID2geneName = {}
            ...
            if len(geneSet)==1:
                ...
                try:
                    ...
                    try:
                        ...
                        pass
    pass#print "---"
    pass#print len(sites)
    pass#print len(UGsites)
    pass#print len(MDsites)
...
def annotate_extreme_vals(filename, column_header, trim_val=0, ext_thresh=0.05, output_name = ''):
    ...
    ratio_list = []
    pass#print "Determining boundary values..."
    #------------------------------------------------------------------------------
    # READING RATIOS
    #------------------------------------------------------------------------------
    ...
    pass#print "BOUNDARIES TARGET"
    pass#print target
    ratio_list.sort()
    lo = ratio_list[int(target)]
    hi = ratio_list[int(num_r-target)]
    pass#print "Ratios..."
    pass#print str(num_r)
    pass#print lo
    pass#print hi
    pass#print target
    pass#print str(num_r-target)
...
def extract_regulated(filename, uratio, column_header, sheetname = "Data", trim=True, extreme=False, ext_thresh=0.05, stringent=False):
    ...
    if not extreme:
        ...
        for row in rdr:
            ...
            if counter % 500 == 0:
                pass#print str(counter)
                #pass#print str(float(row[column_header]))
            counter += 1
...
#                self.richText.SetStyle(i, i+1, wx.TextAttr("red", wx.NullColour, self.fp)) 
    ...
    def OnMotion(self, event):
        ...
        #pass#print pos5
        #pass#print self.protein.sequence[pos5[1]:pos5[1]+1]        
    ...
    def OnLeftDown(self, event):
        ...
        pass#print pos5
        pass#print self.protein.sequence[pos5[1]:pos5[1]+1]
        #pass#print self.richText.ClientToScreenXY()
...
def quick_summary(filename, sheetname='Data', score_threshold=0, reverse_text='rev_gi', gene_threshold=0):
    ...
    '''
    pass#print "PSMs"
    pass#print str(count_PSMs(filename, sheetname, score_threshold=score_threshold, reverse_text=reverse_text))
    peptides = count_unique_peptides(filename, sheetname, score_cutoff=score_threshold)
    pass#print "UNIQUE PEPTIDE SEQUENCES"
    pass#print len(peptides)
    pass#print "GENES MAPPED UNIQUELY BY PEPTIDES"
    ugenes = quick_gene_summary(filename, sheetname, True, gene_threshold)
    pass#print len(ugenes)
    pass#print "ALL GENES MAPPED BY PEPTIDES"
    igenes = quick_gene_summary(filename, sheetname, False, gene_threshold)
    pass#print len(igenes)    
...
def retrieve_database(organism):
    ...
    organism = organism.upper()
    kinobase = {}
    pass#print "Loading kinobase in RAM..."
...
def kinase_match(filename, sheetname="Data", flag="Stringent", phoscheck = False, peptide_stringency = 1, organism = "human"):
    ...
    for peptide in sequences:
        ...
        if len(kin_list) == 1:
            ...
            pass#print kin_dict[kin_list[0]]
            pass#print kin_list[0]
...
def cys_cap_evaluation(filename, sheetname="Data", label=False):
    ...
    if label:
        ...
        for member in peplist:
            if member.find("C")>-1:
                cys_count+=1
        pass#print cys_count
        pass#print tot
        pass#print str(float(cys_count)/float(tot))
...
def cys_alkylation_evaluation(filename, sheetname="Data"):
    ...
    pass#print tot
    pass#print str(float(cys_count)/float(tot))
    pass#print str(float(carbam)/float(tot))
    pass#print str(float(mmts)/float(tot))
...
def fast_peptide_search(dbFile, peptides, seqfile, notfind, out):
    ...
    for en in open(seqfile).xreadlines():
        ...
        pro2en[en[0]] = en[1]
        pro2seq[en[0]] = str(en[2])
    pass#print "Connecting to database..."
    conn = sql.connect(dbFile)
    c = conn.cursor()
    ...
    for p in open(peptides).xreadlines(): # Loop through peptide sequences
        ...
        if p:
            ...
            if a:
                for member in a:
                    #(0-id, 1-sequence text, 2-gene text, 3-accession text, 4-start integer, 5-end integer, 6-mc integer, info text)
                    pass#print >>outw, p+"\t"+member[2]+"\t"+member[3]+"\t"+pro2seq[member[3]]+"\t"+str(member[4]+1)+"\t"+str(member[5]+1)+"\t" + member[7]
            else:
                pass#print >>notfindf, p
            ...
            if peptide_class[k] == i:
                ...
                for p in list(set(peptide[k])):
                    ...
                    if protein_class[p] == 3:
                        tp.append(p)
                pass#print >>firstout, str(i)+ '\t' + k + '\t' + ';'.join(list(set(peptide[k]))) + '\t'+ str(len(list(set(peptide[k])))) \
            ...
            if protein_class[k] == i:
                ...
                for p in list(set(protein[k])):
                    ...
                    if peptide_class[p] == 3:
                        tp.append(p)
                pass#print >>secondout, str(i)+ '\t' + k + '\t' + ';'.join(list(set(protein[k]))) + '\t'+ str(len(list(set(protein[k])))) \
                + '\t' + ';'.join(list(set(fp)))+ '|' + ';'.join(list(set(sp))) + '|' + ';'.join(list(set(tp))) + '\t' + str(len(list(set(fp))))+ '|' + str(len(list(set(sp)))) + '|' + str(len(list(set(tp)))) \
                + '\t' + ';'.join(list(set(fp)))+ '\t' + ';'.join(list(set(sp))) + '\t' + ';'.join(list(set(tp))) + '\t' + str(len(list(set(fp))))+ '\t' + str(len(list(set(sp)))) + '\t' + str(len(list(set(tp))))
...
def LysCpeptideSearch(peptides, seqfile, out, notfind ):
    ...
    for p in open(peptides).xreadlines(): # Loop through peptide sequences
        if counter % 10 == 0:
            pass#print str(counter) +' of ' + str(num)
            now = time.clock()
            total = now-st
            av = float(total)/float(counter + 1)
            remaining = num - counter
            pass#print "Estimated " + str(float(remaining * av)/float(60.0)) + ' minutes remaining'
        counter+=1
        p = p.strip()
        if p:
            ...
            for (k,v) in pro2seq.items():
                ...
                for match1 in iterator1:
                    if match1.start() == 0 and p[-1] == "K":
                        pass#print >>outw, p+"\t"+pro2en[k]+"\t"+k+"\t"+v+"\t"+str(match1.start()+1)+"\t"+str(match1.start()+ len(p))+"\tHead"
                        i = 1
                        ii = 1
                        break
                    if match1.start() == 1 and p[-1] == "K":
                        pass#print >>outw, p+"\t"+pro2en[k]+"\t"+k+"\t"+v+"\t"+str(match1.start()+1)+"\t"+str(match1.start()+ len(p))+"\tMHead"
                        i = 1
                        ii = 1
                        break
                if i == 0:
                    ...
                    for match2 in iterator2:
                        if match2.end() == len(v):
                            pass#print >>outw, p+"\t"+pro2en[k]+"\t"+k+"\t"+v+"\t"+str(match2.start()+2)+"\t"+str(match2.start()+ len(p)+1)+"\tEnd"
                            i = 1
                            ii = 1
                            break
                if i == 0 and p[-1] == "K":
                    ...
                    for match3 in iterator3:
                        pass#print >>outw, p+"\t"+pro2en[k]+"\t"+k+"\t"+v+"\t"+str(match3.start()+2)+"\t"+str(match3.start()+ len(p)+1)+"\tReguler"
                        i = 1
                        ii = 1
                        break
            if ii ==0:
...
def motif_analysis(filename, sheetname="Data", motif = "AKT"):
    ...
    pass#print "WORKING ON... " + file
        
    for i, row in enumerate(rdr):
        if i % 50 == 0:
            pass#print "working on row... " + str(i)
        seq = row["Peptide Sequence"]
        varmod = row["Variable Modifications"]
        if not varmod:
            varmod = ''
...
def pilot_combine_CID_HCD_Lo_HCD_Hi(cid_sheet, hcd_ltq_sheet, hcd_ft_sheet):
    ...
    '''
    pass#print "READ CID"
    pass#print cid_sheet
    cid_rdr = mzReport.reader(cid_sheet, sheet_name = 'fdr')
    pass#print "READ HCD-FT"
    pass#print hcd_ft_sheet
    hcd_ft_rdr = mzReport.reader(hcd_ft_sheet, sheet_name = 'fdr')
    pass#print "READ HCD-LTQ"
    pass#print hcd_ltq_sheet
    ...
    for k in rows:
        ...
        if len(rows[k]) == 1:
            ...
            #pass#print str(rows[k])
            #pass#print str(sorted(rows[k], key=lambda r: r['Peptide Score']))
            ...
            #pass#print len(sorted(rows[k], key=lambda r: r['Conf']))
